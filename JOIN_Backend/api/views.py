...
class ContactView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        existing_contact_ids = set(existing_contacts.values_list('contactId', flat=True))

        incoming_contact_ids = {contact.get('contactId') for contact in contacts_data if 'contactId' in contact}

        contact_ids_to_delete = existing_contact_ids - incoming_contact_ids

        if contact_ids_to_delete:
            deleted_count, _ = Contact.objects.filter(contactId__in=contact_ids_to_delete, user=user).delete()
        ...
        for contact_data in contacts_data:
            ...
            if 'contactId' in contact_data and contact_data['contactId'] in existing_contact_ids:
                try:
                    existing_contact = Contact.objects.get(contactId=contact_data['contactId'], user=user)
